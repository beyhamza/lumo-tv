 ...
 * `when`, so the test can walk it. The phone holds the same map for the same
 * routes — one vocabulary for both applications, see `LumoTvNavHost`.
 *
 ...
 * deep link; since US-017 the home screen produces it every day — a long press on
 * a "Continue" card opens a film or a series straight over Home. The rule is the same one: back to
