...
/**
 * Every stable error code this API emits, ingestion codes included.  Grouped by concern:  - **Generic** — `VALIDATION_FAILED`, `UNAUTHENTICATED`,   `ACCESS_TOKEN_EXPIRED`, `FORBIDDEN`, `NOT_FOUND`, `CONFLICT`,   `RATE_LIMITED`, `INTERNAL_ERROR`. - **Account and sign-in** — `EMAIL_ALREADY_REGISTERED`,   `INVALID_CREDENTIALS`, `PASSWORD_TOO_WEAK`, `OAUTH_TOKEN_INVALID`,   `VERIFICATION_TOKEN_INVALID`, `VERIFICATION_TOKEN_EXPIRED`,   `RESET_TOKEN_INVALID`, `RESET_TOKEN_EXPIRED`. - **Sessions** — `REFRESH_TOKEN_INVALID`, `REFRESH_TOKEN_REUSED`,   `DEVICE_NOT_FOUND`. - **TV activation (RFC 8628)** — `AUTHORIZATION_PENDING`, `SLOW_DOWN`,   `ACCESS_DENIED`, `EXPIRED_TOKEN`, `DEVICE_CODE_NOT_FOUND`,   `DEVICE_CODE_EXPIRED`, `DEVICE_CODE_ALREADY_USED`. - **Sources and ingestion** — `SOURCE_NOT_FOUND`, `SOURCE_NOT_READY`,   `SOURCE_SYNC_IN_PROGRESS`, `SOURCE_SYNC_RATE_LIMITED`, plus every   `IngestionErrorCode`. - **Catalogue and user data** — `CHANNEL_NOT_FOUND`,   `VOD_ITEM_NOT_FOUND`, `SERIES_NOT_FOUND`, `EPISODE_NOT_FOUND`,   `FAVORITE_NOT_FOUND`, `FAVORITE_ALREADY_EXISTS`,   `FAVORITE_GROUP_NOT_FOUND`, `FAVORITE_GROUP_ALREADY_EXISTS`,   `FAVORITE_GROUP_NOT_DELETABLE`.    The last one is only ever the default group. It is separate from a   bare `CONFLICT` for the same reason as the quota codes below: a client   that cannot tell \"this one in particular cannot go\" from \"something   clashed\" has nothing useful to say to the person holding the remote. - **Programme guide** — `EPG_WINDOW_TOO_LARGE`, a `422` from   `GET /sources/{id}/epg` when the batch over the window exceeds one of   its two ceilings. Its own code rather than `VALIDATION_FAILED`   because the request was well-formed and the fix is different: not   \"correct the parameters\" but \"ask for less at a time\". - **Plan limits and billing** — `SOURCE_LIMIT_REACHED`,   `DEVICE_LIMIT_REACHED`, `ALREADY_SUBSCRIBED`,   `BILLING_CUSTOMER_NOT_FOUND`.    The first two exist so that hitting a quota is never reported as a   bare `CONFLICT`. A client that cannot tell \"you already have this\" from   \"your plan stops here\" cannot offer the one thing that resolves the   second: removing something, or upgrading. 
 */
...
public enum ErrorCode {
  ...
  VOD_ITEM_NOT_FOUND("VOD_ITEM_NOT_FOUND"),
  ...
  FAVORITE_GROUP_NOT_DELETABLE("FAVORITE_GROUP_NOT_DELETABLE"),
  
  EPG_WINDOW_TOO_LARGE("EPG_WINDOW_TOO_LARGE"),
