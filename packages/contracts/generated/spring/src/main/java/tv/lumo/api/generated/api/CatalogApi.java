...
public interface CatalogApi {
     ...
     *         or The range is invalid — &#x60;to&#x60; not strictly after &#x60;from&#x60;, or wider than 96 hours (&#x60;VALIDATION_FAILED&#x60;).  (status code 400)
    ...
    /**
     * GET /sources/{id}/epg : Programme guide for a batch of channels of one source, in one call
     ...
     * @return The complete answer for the batch: one entry per requested channel, in the order requested, plus the import status of the guide.  (status code 200)
     *         or &#x60;channelIds&#x60; is empty, has a duplicate or a malformed identifier, or holds more than 100; or the window is not strictly positive or is wider than 96 hours (&#x60;VALIDATION_FAILED&#x60;). Nothing is read.  (status code 400)
     *         or Missing, malformed or expired access token (&#x60;UNAUTHENTICATED&#x60;, &#x60;ACCESS_TOKEN_EXPIRED&#x60;). On &#x60;ACCESS_TOKEN_EXPIRED&#x60; the client refreshes once and replays the request.  (status code 401)
     *         or Two codes, checked in this order:  - &#x60;SOURCE_NOT_FOUND&#x60; — no such source on this account. Absent and   owned by someone else are the same answer, so the operation   cannot be used to probe for source ids; - &#x60;CHANNEL_NOT_FOUND&#x60; — at least one identifier in &#x60;channelIds&#x60; is   not a channel of this source. The whole batch is refused and the   answer does not say which identifier failed, for the same reason.  (status code 404)
     *         or The batch over this window exceeds one of the two ceilings — 5 000 programme occurrences or 4 MiB of JSON (&#x60;EPG_WINDOW_TOO_LARGE&#x60;). **No programme is returned**, so a client never mistakes a partial grid for a complete one. Ask again for fewer channels or a narrower window. Both ceilings were measured before being fixed (docs/releases/0.2.0/s9-00-epg-bench.md): fifty channels over three hours fits several times over; a hundred channels over four days does not, and is meant to be split.  (status code 422)
     */
    @RequestMapping(
        method = RequestMethod.GET,
        value = "/sources/{id}/epg",
        produces = { "application/json", "application/problem+json" }
    )
    
    ResponseEntity<EpgGrid> getSourceEpg(
         @PathVariable("id") UUID id,
        @NotNull @Size(min = 1, max = 100)  @Valid @RequestParam(value = "channelIds", required = true) List<UUID> channelIds,
         @Valid @RequestParam(value = "from", required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) @Nullable OffsetDateTime from,
         @Valid @RequestParam(value = "to", required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) @Nullable OffsetDateTime to
    );
